...
export default function FasonScreen() {
  const route = useRoute<RouteProp<RouteParams, 'Fason'>>();
  ...
  useEffect(() => {
    const fetchFisler = async () => {
      setLoading(true);
      try {
        const url = `https://apicloud.womlistapi.com/api/SabitFis/FisListesi/${depoId}/7/${hareketTipi}`;
  ...
  const handleSec = (item: FisItem) => {
    navigation.navigate('FasonDetay', {
      fisId: item.fisId,
      fisNo: item.fisNo,
