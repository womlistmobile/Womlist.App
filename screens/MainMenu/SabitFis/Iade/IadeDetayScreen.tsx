...
export default function IadeDetayScreen() {
  const route = useRoute<RouteProp<RootStackParamList, 'IadeDetay'>>();
  ...
  const handleVeriTransferi = () => {
    ...
    navigation.navigate('IadeEkBilgiler', {
      fisId,
      depoId,
      userId,
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerText}>İade Detay</Text>
        </View>
            ...
            <>
              ...
              {activeTab === 'Okunan' && (
                <>
                  ...
                  <FlatList data={satirlar.filter((s) => s.okutulanMiktar > 0)} renderItem={({ item }) => renderItem(item, true)} keyExtractor={(item) => item.satirId} />
                </>
              )}
              ...
              {activeTab === 'Kontrol' && (
                <>
                  {satirlar.filter((s) => s.okutulanMiktar > 0).length === 0 ? (
                    ...
                    <>
                      <FlatList data={satirlar.filter((s) => s.okutulanMiktar > 0)} renderItem={({ item }) => renderItem(item)} keyExtractor={(item) => item.satirId} />
