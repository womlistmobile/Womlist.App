...
export default function IadeEkBilgilerScreen() {
  ...
  const handleGonder = async () => {
    ...
    const requestBody = {
      ...
      satirlar: selectedItems.map((item: any) => ({
        depoId: item.depoId?.toUpperCase?.() || depoId.toUpperCase?.(),
        adresId: '',
        stokId: '',
        sayimHareketId: '',
    ...
    try {
      ...
      if (!rawText) throw new Error('Sunucu boş cevap döndü.');

      const result = JSON.parse(rawText);

      if (result.durum) {
        Alert.alert('Başarılı', result.message || 'Veriler gönderildi.');
        navigation.goBack();
      } else {
        Alert.alert('Hata', result.message || 'Veri gönderilemedi.');
      }
    } catch (error: any) {
