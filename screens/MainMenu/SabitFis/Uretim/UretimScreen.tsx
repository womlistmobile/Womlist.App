...
export default function UretimScreen() {
  const route = useRoute<RouteProp<RouteParams, 'Uretim'>>();
  ...
  useEffect(() => {
    const fetchFisler = async () => {
      setLoading(true);
      try {
        const url = `https://apicloud.womlistapi.com/api/SabitFis/FisListesi/${depoId}/6/${hareketTipi}`;
  ...
  const handleSec = (item: FisItem) => {
    navigation.navigate('UretimDetay', {
      fisId: item.fisId,
      fisNo: item.fisNo,
