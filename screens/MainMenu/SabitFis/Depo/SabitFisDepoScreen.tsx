...
export default function SabitFisDepoScreen() {
  const route = useRoute<RouteProp<RouteParams, 'SabitFisDepo'>>();
  ...
  useEffect(() => {
    const fetchFisler = async () => {
      setLoading(true);
      try {
        const url = `https://apicloud.womlistapi.com/api/SabitFis/FisListesi/${depoId}/2/${hareketTipi}`;
